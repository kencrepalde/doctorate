...
rept <- minirep %>% filter(repyn == 2 & index=="2011") %>% 
        summarize(rep = n()) %>% as.data.frame(.)
rept <- minirep %>% filter(repyn == 2 & index=="2012") %>% 
        summarize(rep = n()) %>% union(rept,.) 
rept<-  minirep %>% filter(repyn == 2 & index=="2013") %>% 
        summarize(rep = n()) %>% union(rept,.)
rept<-  minirep %>% filter(repyn == 2 & index=="2014") %>% 
        summarize(rep = n()) %>% union(rept,.)
rept<-  minirep %>% filter(repyn == 2 & index=="2015") %>% 
        summarize(rep = n()) %>% union(rept,.)
rept<-  minirep %>% filter(repyn == 2 & index=="2016") %>% 
        summarize(rep = n()) %>% union(rept,.)
rept<-  minirep %>% filter(repyn == 2 & index=="2017") %>% 
        summarize(rep = n()) %>% union(rept,.)
rept<-  minirep %>% filter(repyn == 2 & index=="2018") %>% 
        summarize(rep = n()) %>% union(rept,.)
